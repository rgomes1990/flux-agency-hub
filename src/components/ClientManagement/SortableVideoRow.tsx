...
export function SortableVideoRow({
  item,
  groupId,
  index,
  selectedItems,
          ...
          <div key={column.id} className="w-44 p-2 border-r border-gray-200">
            {column.type === 'status' ? (
              <StatusButton
                ...
                statuses={statuses}
                onStatusChange={(newStatus) => onUpdateItemStatus(item.id, column.id, newStatus)}
              />
            ) : (
              <Input
                value={item[column.id] || ''}
                onChange={(e) => onUpdateItemStatus(item.id, column.id, e.target.value)}
                className="text-sm"
